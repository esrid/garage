...
func NewHandler(reader PlanningReader) *Handler {
...
func (h *Handler) Page(w http.ResponseWriter, r *http.Request) {
	...
	page.Render(w, r, status, PlanningPage(data))
}
...
func (h *Handler) Fragment(w http.ResponseWriter, r *http.Request) {
	data, status := h.load(r)
	page.Render(w, r, status, PlanningDay(data))
}
...
func (h *Handler) load(r *http.Request) (Day, int) {
	ctx := r.Context()
	query := r.URL.Query()
...
func (h *Handler) slots(ctx context.Context, day appointment.Day, minutes int) ([]Slot, error) {
...
func (h *Handler) rescheduleSlots(ctx context.Context, day appointment.Day, data Day, pageMinutes int, pageSlots []Slot) map[int][]Slot {
	...
	options := make(map[int][]Slot, len(lengths))
...
func planningMinutes(raw string) (int, []string) {
	...
	if err != nil || !slices.Contains(PlanningDurations, minutes) {
		return defaultPlanningMinutes, []string{"Durée non proposée : affichage en 1 h."}
	}
	return minutes, nil
}

func (h *Handler) unavailable(ctx context.Context, err error, minutes int, alert string) (Day, int) {
