...
func (h *Calls) Day(w http.ResponseWriter, r *http.Request) {
	...
	page.Render(w, r, http.StatusOK, CallsPage(history))
}
...
func (h *Calls) One(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.As(err, &notFound) {
			...
			page.Render(w, r, http.StatusNotFound, CallProblemPage(
				"Appel introuvable",
		...
		page.Render(w, r, http.StatusOK, CallProblemPage(
			"Appel indisponible",
			"Cet appel n'a pas pu être lu. Réessayez dans un instant.",
		))
		return
	}
	page.Render(w, r, http.StatusOK, CallPage(call))
}
