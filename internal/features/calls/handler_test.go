...
func newTestCalls(reader CallHistoryReader) *Calls {
	h := NewCalls(reader)
	h.now = func() time.Time { return callsNow }
	return h
}
...
// Martinique as a fixed zone: these tests must not depend on the tzdata of the
// machine running them, and the island has no daylight saving.
var martinique = time.FixedZone("AST", -4*60*60)

var callsNow = time.Date(2026, 7, 30, 9, 30, 0, 0, martinique)
