...
- 2026-07-30 · Agent B · **branche `feat/foundation-postgres-css` mergée dans `main`** sur feu vert du fondateur, poussée (`764ed9e`). Avant le merge, la suite complète tournée contre une base **vierge** a révélé un bloqueur de release invisible jusque-là : `00008` recréait `conversations_tenant_started_idx`, déjà créé par `00006`. Toute base neuve échouait à la migration 8 et l'application sortait au démarrage ; seules les bases de dev, créées avant que la ligne d'index n'entre dans `00006`, fonctionnaient — d'où une suite verte sur une release cassée. Index dupliqué supprimé (`2633a43`). Vérifié sur `postgres:18.4-bookworm` neuf : migrations jusqu'à la version 9, suite `-race` verte sans skip, application qui boote et sert `/`, `/healthz`, `/readyz`, `/login`, `/robots.txt` en 200. **Leçon** : ne plus jamais valider une migration sur une base déjà migrée.
- 2026-07-30 · Agent B · doctrine d'architecture écrite : `docs/ARCHITECTURE.md` fait autorité (un dossier par capacité métier, `platform/` pour l'infra, `app/` pour la composition, 15 règles). `AGENTS.md` y renvoie, et le §10 de `docs/ARCHITECTURE-AUDIT.md` défère à ce guide au lieu de proposer un second arbre cible. Le code est encore en `core/features/adapters/web` : le guide est la cible, l'audit est la carte entre les deux.
